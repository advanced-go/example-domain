...
func Test_timeseriesV2Handler(t *testing.T) {
	...
	for _, tt := range tests {
		failures, req, resp := httpxtest.ReadHttp("file://[cwd]/servicetest/timeseriesv2/", tt.args.req, tt.args.resp)
		if failures != nil {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			timeseriesHandlerV2[core.Output](w, req)
			...
			if w.Result().StatusCode != resp.StatusCode {
				...
				var content bool
				failures, content, gotT, wantT = httpxtest.Content[[]entryv2.Entry](w.Result(), resp, timeseriesV2TestBytes)
				if failures != nil {
...
func timeseriesV2TestBytes(got *http.Response, gotBytes []byte, want *http.Response, wantBytes []byte) []httpxtest.Args {
	//fmt.Printf("got = %v\n[len:%v]\n", string(gotBytes), len(gotBytes))
	//fmt.Printf("want = %v\n[len:%v]\n", string(wantBytes), len(wantBytes))
	return nil
}

func timeseriesV2Errorf(t *testing.T, failures []httpxtest.Args) {
