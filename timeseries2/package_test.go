...
func Example_HttpHandler() {
	...
	// Bad Request
	uri := "http://localhost:8080/github.com/advanced-go/example-domain/timeseries2/entry"
	...
	// Resource Not Found
	uri = "http://localhost:8080/github.com/advanced-go/example-domain/timeseries2:invalid"
	r, _ = http.NewRequest("GET", uri, nil)
	w = http2.NewRecorder()
	HttpHandler(w, r)
	buf, status = io2.ReadAll(w.Result().Body)
	...
	//Output:
	//test: HttpHandler() -> [status:400] [content:error invalid path, not a valid URN: /github.com/advanced-go/example-domain/timeseries2/entry]
